...
The three character-specific cues below are original deterministic synthesis generated by
...
The original character-specific cues are mono PCM WAV at 44,100 Hz and are intentionally kept uncompressed
because all three together are under 250 KB. `SfxBank` checks `<key>.ogg` and then `<key>.wav`. If a file is
